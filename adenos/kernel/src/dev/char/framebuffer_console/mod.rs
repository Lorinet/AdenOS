...
where F: Framebuffer {
    ...
    fn _write_one(&mut self, val: u8) -> Result<(), Error> {
        match val {
            b'\n' => {
                self._new_line();
            },
...
impl<F> Device for FramebufferConsole<F>
where F: Framebuffer {
...
impl<F> Write for FramebufferConsole<F>
where F: Framebuffer {
...
impl<F> ConsoleDevice for FramebufferConsole<F>
where F: Framebuffer {
