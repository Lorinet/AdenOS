...
pub fn init_logger(framebuffer: &'static mut [u8], info: FrameBufferInfo) {
    unsafe {
        logger::LOGGER = Some(Mutex::new(logger::Logger::new(framebuffer, info)));
    }
    println!("Framebuffer info: {:?}", info);
}
    ...
    D: LegacyMemoryRegion,
{
    ...
    .expect("no entry pointAAAAA");
    println!("Entry point at: {:#x}", entry_point.as_u64());
    ...
    let recursive_index = if CONFIG.map_page_table_recursively {
        println!("Map page table recursively");
        let index = CONFIG
            .recursive_index
