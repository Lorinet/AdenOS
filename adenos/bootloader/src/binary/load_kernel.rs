    ...
    F: FrameAllocator<Size4KiB>,
{
    fn handle_load_segment(&mut self, segment: ProgramHeader) -> Result<(), &'static str> {
        println!("Handling Segment: {:x?}", segment);
        ...
        for frame_addr in (start_frame.start_address().as_u64()..=end_frame.start_address().as_u64()).step_by(0x1000) {
            let frame = PhysFrame::from_start_address(PhysAddr::new(frame_addr)).unwrap();
            let offset = frame - start_frame;
            let page = start_page + offset;
            //println!("{:#x} {:#x} {:#x}", page.start_address(), frame.start_address(), segment_flags);
            if let Err(_) = self.page_table.translate_page(page) {
                let flusher = unsafe {
                    self.page_table
                        .map_to(page, frame, segment_flags, self.frame_allocator)
                        .map_err(|_err| { println!("{:?}", _err); "map_to failed" })?
                };
