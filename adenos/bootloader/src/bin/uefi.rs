...
fn efi_main(image: Handle, st: SystemTable<Boot>) -> Status {
    ...
    println!("Hello World from UEFI bootloader!");
    println!("Using framebuffer at {:#x}", framebuffer_addr);
...
fn create_page_tables(
    ...
    let (kernel_page_table, kernel_level_4_frame) = {
        ...
        println!("New page table at: {:#?}", &frame);
...
fn panic(info: &PanicInfo) -> ! {
    unsafe {
        bootloader::binary::logger::LOGGER.as_mut().unwrap().force_unlock();
    };
    println!("\n\nBOOTLOADER ERROR\n================\n\nAdenOS has encountered an unrecoverable error while starting up. Your system has been shut down.\n\n{}", info);
    loop {
        unsafe { asm!("cli; hlt") };
    }
}
